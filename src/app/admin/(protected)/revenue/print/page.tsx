...
export default async function RevenuePrintPage({
  ...
  const res = await fetch(`${origin}/api/revenue?${params.toString()}`, {
    cache: "no-store",
    headers: {
      cookie: h.get("cookie") ?? "",
    },
  ...
  if (!res.ok) {
    ...
    return (
      <div className="bg-white p-6 text-stone-900 print:p-0">
  ...
  const periodLabel =
    dateFrom && dateTo
      ? `${new Date(dateFrom).toLocaleDateString(
          "nl-NL"
        )} t/m ${new Date(dateTo).toLocaleDateString("nl-NL")}`
      : "Periode";
  ...
  return (
    <div className="bg-white p-6 text-stone-900 print:p-0">
      <header className="mb-4 border-b border-stone-200 pb-3 print:mb-2 print:pb-2">
        ...
        <p className="text-sm text-stone-600">
          Status: <b>{status}</b> · Datumtype: <b>Speeldatum</b> · Periode:{" "}
          <b>{periodLabel}</b>
          {partnerSlug ? (
            <>
              {" "}
              · Partnerfilter: <b>{partnerSlug}</b>
            </>
          ) : null}
        </p>
      ...
      <section className="mb-4 grid grid-cols-2 gap-2 text-sm print:mb-3 print:gap-1 md:grid-cols-4">
        <KPI
          label="Totaal omzet"
          value={euro(
            data.summary.totalTurnoverCents,
            data.summary.currency
          )}
        />
        <KPI
          label="Fee D-EscapeRoom"
          value={euro(
            data.summary.totalPlatformFeeCents,
            data.summary.currency
          )}
        />
        <KPI
          label="Fee hondenschool"
          value={euro(
            data.summary.totalPartnerFeeCents,
            data.summary.currency
          )}
        />
        <KPI
          label="Kortingen"
          value={euro(
            data.summary.totalDiscountsCents,
            data.summary.currency
          )}
        />
      </section>
        ...
        <table className="w-full border-collapse text-[12px] leading-5">
          ...
          <tbody>
            {data.items.map((it) => (
              <tr
                key={it.id}
                className="border-t border-stone-200 print:border-stone-100"
              >
                <Td>
                  {it.date
                    ? new Date(it.date).toLocaleDateString("nl-NL")
                    : "-"}
                </Td>
                ...
                <Td className="text-right">
                  {euro(it.totalAmountCents, it.currency)}
                </Td>

                <Td className="text-right">
                  {euro(it.platformFeeCents, it.currency)}
                </Td>

                <Td className="text-right">
                  {euro(it.partnerFeeCents, it.currency)}
                </Td>

                <Td className="text-right">
                  {euro(it.discountCents, it.currency)}
                </Td>
...
function Th({
  ...
  return (
    <th
      className={`px-2 py-2 text-left text-[11px] font-semibold ${className}`}
    >
      {children}
    </th>
  );
...
function Td({
  ...
  return (
    <td className={`px-2 py-2 align-top text-stone-900 ${className}`}>
      {children}
    </td>
  );
}