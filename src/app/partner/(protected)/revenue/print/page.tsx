...
// Geen caching/ISR voor deze route
...
type RevenueItem = {
  id: string;
  date: string | null;               // slot.startTime (ISO)
...
export default function RevenuePrintPage() {
  ...
  // Query-params (partnerSlug is essentieel vanuit partner-omgeving)
  const partnerSlug = sp.get("partnerSlug") || "";
  const status = (sp.get("status") || "CONFIRMED").toUpperCase();
  const dateFrom = sp.get("dateFrom") || "";
  const dateTo = sp.get("dateTo") || "";
  const timeField = "slot"; // altijd speeldatum
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        const r = await fetch(`/api/revenue?${params.toString()}`, {
          cache: "no-store",
          credentials: "include",
        });
        ...
        // Kleine delay zodat fonts/layout klaar zijn
  ...
  // Zelfde default-sort als admin (datum DESC op speeldatum)
  ...
  // Teruggestort per regel + totaaltje (alleen zichtbare regels)
  const refundedOf = (it: RevenueItem) =>
    (it.status?.toUpperCase() === "REFUNDED" ? (it.refundedAmountCents ?? it.platformFeeCents) : 0) || 0;

  const totalRefundsCents = React.useMemo(
    () => itemsSorted.reduce((s, it) => s + refundedOf(it), 0),
    [itemsSorted]
  );
    ...
    <div className="bg-white text-stone-900 p-6 print:p-0">
      ...
      <header className="mb-4 border-b border-stone-200 pb-3 print:mb-2 print:pb-2">
        ...
        <p className="text-sm text-stone-600">
          Status: <b>{status}</b>
          {partnerSlug ? <> · Partner: <b>{partnerSlug}</b></> : null}
          {" "}· Datumtype: <b>Speeldatum</b> · Periode: <b>{periodLabel}</b>
        </p>

        {/* Alleen zichtbaar op scherm */}
            ...
            <table className="w-full border-collapse text-[12px] leading-5">
              ...
              <tbody>
                {itemsSorted.map((it) => (
                  <tr key={it.id} className="border-t border-stone-200 print:border-stone-100">
                    ...
                    <Td className="max-w-[240px] truncate" title={it.customerName ?? undefined}>
                      {it.customerName ?? "-"}
                    </Td>
                    ...
                    <Td>{it.status}</Td> {/* status blijft EN voor 1:1 export/vergelijking */}
                    <Td className="text-right">
                      {refundedOf(it) > 0 ? euro(refundedOf(it), it.currency) : "—"}
                    </Td>
                  </tr>
                ))}
              </tbody>
              <tfoot className="bg-stone-50 font-semibold text-stone-800 print:bg-white">
                <tr>
                  ...
                  <Td children={undefined} />
                  <Td className="text-right">{euro(totalRefundsCents, data.summary.currency)}</Td>
                </tr>
              </tfoot>
            </table>
          </section>
...
function Td({ children, className = "", title }: { children: React.ReactNode; className?: string; title?: string }) {
  return <td className={`px-2 py-2 align-top text-stone-900 ${className}`} title={title}>{children}</td>;
}
