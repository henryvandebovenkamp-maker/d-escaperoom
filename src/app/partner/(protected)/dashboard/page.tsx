...
function startOfNextAmsterdamMonth(date = new Date()) {
  ...
  if (month === 12) {
    return amsterdamLocalDateToUtc(year + 1, 1, 1);
  }

  return amsterdamLocalDateToUtc(year, month + 1, 1);
}
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-0.5 text-sm text-stone-700">
              Overzicht van jouw boekingen, omzet en planning.
            </p>
          </div>
      ...
      <section className="grid grid-cols-1 gap-4 md:grid-cols-3">
        ...
        <StatCard title="GEPUBLICEERDE SLOTS" value={bookableSlots} hint="Toekomstige boekbare slots" icon="🎯" href="/partner/slots" />
        <StatCard title="OMZET OP LOCATIE" value={euro(partnerRevenueTotal)} hint={`Deze maand: ${euro(partnerRevenueMonth)}`} icon="🐾" accent="from-emerald-500/10 to-emerald-600/10" href="/partner/revenue" />
        ...
        <StatCard title="AANBETALINGEN" value={euro(feeTotal)} hint={`Deze maand: ${euro(feeMonth)}`} icon="💶" href="/partner/revenue" />
      </section>
      ...
      <section className="grid grid-cols-1 gap-4 md:grid-cols-3">
        <div className="rounded-xl border border-stone-200 bg-white p-4 md:col-span-2">
          ...
          <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4">
            <Link href="/partner/slots" className="rounded-lg border bg-stone-50 px-3 py-2 text-xs font-medium text-stone-800 transition hover:bg-stone-100">
              📅 Tijdsloten
            </Link>
            <Link href="/partner/agenda" className="rounded-lg border bg-stone-50 px-3 py-2 text-xs font-medium text-stone-800 transition hover:bg-stone-100">
              🗓️ Agenda
            </Link>
            <Link href="/partner/revenue" className="rounded-lg border bg-stone-50 px-3 py-2 text-xs font-medium text-stone-800 transition hover:bg-stone-100">
              📈 Omzetcijfers
            </Link>
            <Link href="/partner/profile" className="rounded-lg border bg-stone-50 px-3 py-2 text-xs font-medium text-stone-800 transition hover:bg-stone-100">
              🧭 Profiel
            </Link>
          </div>
        </div>
